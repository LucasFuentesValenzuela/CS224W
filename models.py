...
from predictors import LinkPredictor, MADpredictor, FieldType
...
class MAD_GCN(nn.Module):
    def __init__(
        ...
        if field_NN == FieldType.EXTERNAL:
            self.field_dim = output_dim
        else:
            self.field_dim = 0 # Field not generated by gcn stuff

        self.embeds = nn.Embedding(
            embedding_shape[0], embedding_dim*self.n_heads)
        nn.init.xavier_uniform_(self.embeds.weight)

        conv_layers = [GCNConv(embedding_dim*self.n_heads, (hidden_dim + self.field_dim)*self.n_heads, cached=cache)] + \
            [GCNConv((hidden_dim + self.field_dim)*self.n_heads, (hidden_dim + self.field_dim)*self.n_heads, cached=cache) for _ in range(num_layers-2)] + \
            [GCNConv((hidden_dim + self.field_dim)*self.n_heads, (self.output_dim + self.field_dim) *
    ...
    def forward(self, adj_t: torch_sparse.SparseTensor, edges: torch.Tensor) -> torch.Tensor:
        ...
        if self.gcn_cache == None:
            ...
            x = torch.reshape(x, (self.n_nodes, self.output_dim + self.field_dim, self.n_heads))
        ...
        field = None
        if self.field_NN == FieldType.EXTERNAL:
            field = x[:, :, -self.field_dim:]
            x = x[:, :, :self.output_dim]

        return self.predictor(x, edges, field)
...
class MAD_GCN_LRGA(torch.nn.Module):
    def __init__(
        ...
        self.embedding = nn.Embedding(
            embedding_shape[0], embedding_dim*n_heads)

        # convolutional layers
        self.convs = torch.nn.ModuleList()
        self.convs.append(GCNConv(embedding_dim*n_heads, hidden_dim*n_heads, cached=cache))
        # attention layer
        self.attention = torch.nn.ModuleList()
        self.attention.append(LowRankAttention(self.k, embedding_dim*n_heads, dropout))
        # dimension_reduce #TODO: clarify
        self.dimension_reduce = torch.nn.ModuleList()
        self.dimension_reduce.append(nn.Sequential(nn.Linear(2*self.k + hidden_dim*n_heads,
                                                             hidden_dim*n_heads), nn.ReLU()))
        # batch normalization layers
        self.bn = nn.ModuleList([nn.BatchNorm1d(hidden_dim*n_heads)
                                 for _ in range(num_layers-1)])

        for _ in range(num_layers - 1):
            self.convs.append(GCNConv(hidden_dim*n_heads, hidden_dim*n_heads, cached=cache))
            self.attention.append(LowRankAttention(
                self.k, hidden_dim*n_heads, dropout))
            self.dimension_reduce.append(nn.Sequential(nn.Linear(2*self.k + hidden_dim*n_heads,
                                                                 hidden_dim*n_heads)))
        self.dimension_reduce[-1] = nn.Sequential(nn.Linear(2*self.k + hidden_dim*n_heads,
                                                            output_dim*n_heads))
        self.dropout = dropout
